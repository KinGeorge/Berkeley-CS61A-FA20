...
def balanced(m):
    ...
    if is_planet(m):
        ...
        return total_weight(end(left(m))) * length(left(m)) == total_weight(end(right(m)))*length(right(m)) and\
...
def replace_leaf(t, find_value, replace_value):
    ...
    if is_leaf(t) and label(t) == find_value:
        ...
        for branch in branches(t):
            children.append(replace_leaf(branch, find_value, replace_value))
        return tree(label(t), children)
...
def sub_interval(x, y):
    ...
    and any value in y."""
    lower, upper = lower_bound(x) - upper_bound(y), upper_bound(x) - lower_bound(y)
    return interval(lower, upper)
...
def multiple_references_explanation():
    return """Eva Lu Ator is right about the multiple reference problem.
...
def quadratic(x, a, b, c):
    ...
    if extreme_point <= lower_bound(x) or extreme_point >= upper_bound(x):
        lower, upper = min(quadratic_func(lower_bound(x)), quadratic_func(upper_bound(x))),\
                       max(quadratic_func(lower_bound(x)), quadratic_func(upper_bound(x)))
    else:
        lower, upper = min(quadratic_func(lower_bound(x)), quadratic_func(upper_bound(x)), quadratic_func(extreme_point)),\
                       max(quadratic_func(lower_bound(x)), quadratic_func(upper_bound(x)), quadratic_func(extreme_point))
