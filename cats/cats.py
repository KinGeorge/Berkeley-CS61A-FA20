...
def about(topic):
    ...
    def about_topic(paragraphs):
        for i in range(len(topic)):
            if topic[i] in split(lower(remove_punctuation(paragraphs))):
                return True
        return False
...
def accuracy(typed, reference):
    ...
    smaller = type_len if type_len < ref_len else ref_len
...
def report_progress(typed, prompt, user_id, send):
    ...
    message['id'] = user_id
    message['progress'] = correct/len(prompt)
    send(message)
...
def time_per_word(times_per_player, words):
    ...
    for player in times_per_player:
        ...
        for i in range(len(player)-1):
            temp.append(player[i+1]-player[i])
        times.append(temp)
