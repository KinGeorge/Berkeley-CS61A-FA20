...
def coords(fn, seq, lower, upper):
    ...
    [[-2, 4], [1, 1], [3, 9]]
    """
    return [[s, fn(s)] for s in seq if fn(s)<=upper and fn(s)>=lower]
...
def riffle(deck):
    ...
    """
    return [deck[(len(deck)//2)*(k%2)+k//2] for k in range(len(deck))]
...
def add_trees(t1, t2):
    ...
    if len(zipped) > 1:
        result_branches = [add_trees(zipped[i][0],zipped[i][1]) for i in range(1,len(zipped))]
    else:
        result_branches = []
    result_tree = tree(sum(zipped[0]), result_branches)
    return result_tree
